...
После того как все процесситься через **ViT**, используется MLP с двумя слоями и все подается в QWen LLM. При это они не пишут ничего про данные кроме того какие категории используются, при этом они уже успели взять данные PixMo и использовать. Сама модель обучается в 2 шага целиком, и у них есть ещё post-training, где они доучивают модель с помощью Reinforcement Learning.


### LISA
Перейдем к моделям которые, вдохновляются задачами сегментации из компьютерного зрения. Модель LISA - это переход, от того, что нам бы хотелсоь декадировать не только в текст, но и предсказывать например карту сегментации.

Модель LISA решают задачку, которую они назвали reasoning segmentation. Например у нас есть картинка, и какой-то вопрос, в котором требуется либо логическая цепочка, либо знания о мире.
Например, "где на картинке еда с большим количеством витамина C". Т.е нужно найти где во первых здесь еда, потом понять и сравнить в какой еде из представленой больше витамина C и выдать маску сегментации.

На вход у нас картинка и текст, на выход маска сегментации. (при это решается задача сегментации какой-то области.)

Как она работает, у нас есть мультимодальная модель - LLaVA, в которую подается картинка и текст, поверх LLaVa (поскольку это трансформер), накидывают ещё LoRA (которую дообучают, а LLaVA не трогают, чтобы сохранялись какие-то общие знания, с одной стороны, а с другой эта модель тюнится на очень небольшом кол-ве данных, порядка 200 картинок). Затем, важный момент, это то что помимо стандартных текстовых токенов, они добавляют специальный служебный токен, который обозначается <SEG>. Модель моет предсказывать в том числе и этот токен, но в отличие от текста, мы будем его декодировать не в текст, а будем его обрабатывать специальным образом, в тот момент когда модель предсказал токен <SEG> - мы берем его эмбеддинг который ему соответствует и засовываем его в Decoder. На рисунке видно, что картинка ещё обрабатывается Vision Encoder-ом и Vision Decoder-ом который ещё кондишиниться на вот 
тот токен <SEG>. Вот эта часть Vision Encoder и Vision Decoder это относится к модели Segment Anything. И вот в результате из этого декодера мы модем получить требуемую маску (при этом Vision Encoder заморожен). Т.е у нас есть легковесный декодер и есть LoRA и мы чуть чуть адаптируем модель, но при этом за счет вот этого трюка с <SEG> токеном получаем некоторое новое свойство, что можем достаточно легко получать маску для запроса.


### OMG-LLaVA
В задаче сегментации есть разные варианты её постановки, например как в LISA, бывает общая сегментаци изображения, instance сегментация (когда нужно каждый обхект выделить и уникальным айдишником пометить). Авторы берут задачу сегментации из компьютерного зрения и обобщают её так, чтобы при этом сохранились общие умения которые у модели и так есть (то что касается captioning). 

Т.е у нас есть image-level часть - где мы можем делать кэпшининг, можем задавать какие-то вопросы про изображение в целом. Далее у нас возникает задача сегментациии, в зависимости от того кака мы хотим понимать изображение, может возникать интерактивная сегментация, семантическая сегментация, по мимо этого мы можем делать сегментацию с описанием текстом, т.е когда мы виделили сегмент - про него что-то сказали, выделили другой сегмент - про него тоже что-то сказали, и наконец наиболее сложный сценарий когда мы можем задавать какой-то текст и картинку, потом задавать какой-то ввод ( это может быть точка, маска) и просить что-то про этот объект рассказать, либо просегментировать похожий на этот оюъект. т.е у нас уже появляются пользовательские вводы на входе.

Как выглядит вход. У нас есть картинка-текст, далее мы можем задавать какой-то регион, может быть несколько регинов, т.е у нас уже возникает несколько сущностей (есть токены которые соответствуют картинке, тексту и регионам) и точно также модель может отвечать нам тектом и выдвать <SEG> токены в произвольном количестве, чтобы референсить какие-то части изображения.

Модель работает следующим образом, во-первых каринка и регоин подаются в модель **OMG-Seg**, эта модель чисто картиночая, которая может нам посчитать токены для всего изображения (dense-токены), один токен для условно классификации изображения или какой-то высокоуровневой информации, и токены которые могут соответствавать разным объектам, дальше с нашим запросом мы эти токены извлекаем из инормации про нашу картинку, т.е делаем какую-то проекцию, и все это подаем в LLM-ку, т.е у нас LLM получает зеленые токены (это картиночные плотные токены), белые - это токены, которые отвечают каким-то объектам, текстовые - серенькие, а синий токен который соответствует, какой-то области которую мы выделилил, и дальше LLM может выдавать последовательности текстовых токенов и <SEG> токенов, который мы можем, декодировать, чтобы получать маски. Т.е получается что у нас диалог становится более богатым и мы можем и на вход какие-то рефернсы подавать (какую область учесть) и модель на выходе может уже не просто один <SEG> предсказывать, большее их количество (если мы конечно нужным образом обучаем, т.е нужно данные для этого готовить)
