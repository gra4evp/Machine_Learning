...
CLIP ViT-L/14 336px, high-res is processed using overlapped slicing Various LLM

Training the whole model, no freezing:
1) Pretraing on PixMo
2) Finetune on PixMo ans academic datasets

Дизайн модели похож на LLaVA, есть CLIP ViT-L/14 336px, есть high-res с обработкой слайсинга с перекрытием, LLM используются разные как открытые так и менее открытые.

Т.е картинка нарезается, и дальше есть запрос, и по мимо того что модель может выдавать какие-то текстовые описания, они обучают модель так чтобы она могла указать на конкретную точку в картинке (по типу XML картинке, которую она выдает, координаты, и подпись), например <point x="63.5" y="44.5" alt="MtRainer">Mt Rainer</point>. Обучают они всю модель целиком, ничего не замораживают, pretrain они делают на датасете PixMo (по сути это Instruction data), а Finetine они делают на PixMo и на акадимических датасетах. т.е с начала делается претрейн, чтобы модель была больше похожа на чатбота, а потом ещё в нее на втором этапе докидываются академичисекие значния, что бы она была более точна.

#### PixMo (Pixelf for Molmo)
Основая новизна сбора данных в данной работе, это то как получать описание картинок. Допустим естькартинка, и в целом можно было бы попросить человека написать руками, но с одной стороны это долго, с другой уже есть GPT и разметчека знают что это такое, и разметчики могут полениться и засунуть эту картинку в GPT и по сути получается что делается то же самое что делал LLaVa - т.е дисцилируете GPT. Авторы заметили, что людям печатать тяжело, но сдлать голосовое сообщение намного проще - они просят человека, просто описать голосовым сообщением картинку 60 или 90 секунд. Они отмечают два момента, первое - это то что человек описывает картинку очень подробно, намного подробнее чем если бы он печатал, 
