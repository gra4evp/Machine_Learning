...
У нас есть CLIP, что если мы к CLIP-у добавляем ещё какие-то картиночные энкодеры (которые называют Vision Experts), например для задачи Object Detection, или например может быть отдельный энкодер, который учился для задачи Text Recognition, или для задачи сегментации, такой как SegmentAnything.

И тут возникает вопрос, у нас есть энкодеры которые выдают информацию, в признаки в разных разрешениях, как их смерджить? Фьюзить? (Fusion)

Тут есть несколько разных подходов. В обзоре статьи от НВИДИА EAGLE, гворят про следующие:

[<img src="src/pictures/017_Overview_EAGLE_exploration_fusion_paradigms.png" alt="Схема архитектуры Fuyu" style="width: 80%; height: auto; display: block; margin: 0 auto;" />](https://www.adept.ai/blog/fuyu-8b)  
> *Рис. 17: Обзор исследования **Eagle**. В работе исследуются, способы построения MLLMs с несколькими визуальными энкодерами, для определения оптимального дизайна. Проводится систематическое сравнение популярных парадигм fusio.*

- Sequence Append
Во первых мы можем брать эти токены, от разных энкодеров, вытягивать их в полседовательности и конкатенировать. При этом у нас конткест, т.е кол-во токенов которые соответствует картинке будет становится очень большим, это может быть тяжело обучать и на инференсе, это тожет может быть дорого (по времени и попамяти может становится сложно, особенно если у нас высокое разрешение)

- Channel-wise Concatination
|Ещё можно делать ресайз, там где маленькое разрешение, делать его побольше и конкатенировать поконально. Т.е глубина токенов, будет становиться побольше, но дальше засчет, например линейных слоев, эта информация агрегируется. Вполне валидный подлход, который бдуте не на столько дорогим.

 - LLaVA-HR
Если у нас есть признаки, в низком и выскоком разрешении, то мы можем эти признаки в низком разрешении, обогащать с помощью Cross-Attention, т.е это стандартные ViT блоки, где будут некоторые запросы Q в низком разрешении, которые мы обогащаем, значениями в высоком разрешении

- Mini-Gemini
Можно ещё делать, похожие идеи, попытаться информацию в высоком разрешении засунуть в низкое разрешение, в немного другом формате (тут лектор не удосужился пояснить)
...
[<img src="src/pictures/017_Overview_EAGLE_exploration_fusion_paradigms.png" alt="Схема архитектуры Eagle" style="width: 80%; height: auto; display: block; margin: 0 auto;" />](https://www.adept.ai/blog/fuyu-8b)  
